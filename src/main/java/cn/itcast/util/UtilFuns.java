...
public class UtilFuns {
    ...
    static public String joinStr(String[] aStr, String sPrefix, String sSuffix,
        ...
        if (aStr != null) {
            ...
            sBuffer.delete(sBuffer.length() - SplitFlag.length(),
                    sBuffer.length()); // 去掉最后的分隔符 SplitFlag
        } else {
            sBuffer = sBuffer.append("");
        }
        return sBuffer.toString();
     ...
     */
    public static void formatArrayList(ArrayList<String> al) {
     ...
     * 输入参数：String CurrentValue 查找出的数据库中的数据 String[] content 需要输出的所有下拉列表框的内容 <br>
     * 输出参数：返回下拉列表 <br>
     ...
     */
    public String ComboList(String CurrentValue, String[] values,
                            String[] content) {
        int i = 0;
        StringBuffer sBuf = new StringBuffer();
        String selected = " selected";
    ...
    public String ComboListMust(String CurrentValue, String[] values,
                                String[] content) {
        int i = 0;
        StringBuffer sBuf = new StringBuffer();
        String selected = " selected";
    ...
    public static String StrToDateTimeFormat(String timestampStr, String pattern)
            throws ParseException {
        String s = "";
        try {
    ...
    public static String formatDateTimeCN(String date) throws ParseException {
        ...
        try {
            ...
            if (date.length() == 4) { // yyyy
                ...
                s = date.replaceAll("-0", "-").replaceFirst("-", "年")
                        .replaceFirst("-", "月")
                        + "日";
                ...
                s = date.replaceAll(":0", ":").replaceFirst(":", "时")
                        .replaceFirst(":", "分")
                        + "秒";
            } else if (date.length() == 13) { // yyyy-MM-dd HH
                s = date.replaceAll("-0", "-").replaceFirst("-", "年")
                        .replaceFirst("-", "月").replaceAll(" 0", " ")
                        .replaceFirst(" ", "日")
                        + "时";
            } else if (date.length() == 16) { // yyyy-MM-dd HH:mm
                s = date.replaceAll("-0", "-").replaceFirst("-", "年")
                        .replaceFirst("-", "月").replaceAll(" 0", " ")
                        .replaceFirst(" ", "日").replaceAll(":0", ":")
                        .replaceFirst(":", "时")
                        + "分";
    ...
    public String htmlReplaceAll(String s) {
        try {
            ...
            sBuf.append(s.replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;")
                    .replaceAll(">", "&gt;").replaceAll("\"", "&quot;")
                    .replaceAll("\n", "<br\\>"));
            return sBuf.toString();
        } catch (Exception e) {
     ...
     */
    public static String getPassString(String strvalue, int Flag, int sFlag,
                                       int iPassLen) {
        try {
     ...
     */
    public static String getPatternString(String s, int iStartPos, int iEndPos,
                                          String sPattern, int iPatternLen) {
        try {
            if (iEndPos == 0) {
                iEndPos = s.length();
            }
    ...
    public static String getPatternString(String s, int iStartPos,
                                          String sPattern, int iPatternLen) {
    ...
    public static String getQQString(String strvalue) {
        try {
            ...
            if (strvalue.equals("")) {
                ...
                QQ = "<img src=\"http://wpa.qq.com/pa?p=1:"
                        + strvalue
                        + ":4\">"
                        + " <SPAN title=\"有事叫我!\" style=\"CURSOR: hand\""
                        + " onclick=\"window.open('http://wpa.qq.com/msgrd?V=1&amp;Uin="
                        + strvalue + "&amp;Site=21pan&amp;Menu=yes')\">"
                        + strvalue + "</SPAN>";
            }
            strvalue = QQ;
            // System.out.print( "strvalue:" + strvalue );
    ...
    public String getNoExistString(String allString, String existString) {
        return this
                .getNoExistString(UtilFuns.splitStr(allString, ","), existString);
    }
    ...
    public static void main(String[] args) throws Exception {
		 ...
		 * sBuffer.append
		 * ("select User_P.loginname,").append(sTableName).append(".createby,count("
		 * ).append(sTableName).append(".createby)")
		 * .append(" from ").append(sTableName).append("")
		 * .append(" right join") .append(" User_P")
		 * .append(" on User_P.loginname="
		 * ).append(sTableName).append(".createby") .append(" where 1=1"); if
		 * (find_Name.compareTo("")!=0){
		 * sBuffer.append(" and ").append(sTableName
		 * ).append(".createby='").append(sTableName).append("'"); } if
		 * (find_DateStart.compareTo("")!=0){
		 * sBuffer.append(" and createdate<='"
		 * ).append(find_DateStart).append(" 00:00:00'"); } if
		 * (find_DateStart.compareTo("")!=0){
		 * sBuffer.append(" and createdate>='"
		 * ).append(find_DateEnd).append(" 23:59:59'"); }
		 * sBuffer.append(" group by ").append(sTableName).append(".createby")
		 * .append(";");
		 * 
		 * 
		 * //Step 3 read updatenum
		 * sBuffer.append("select count(updateby) from ")
		 * .append(sTableName).append("")
		 * .append(" where ").append(sTableName).append(".updateby=''")
		 * .append(" and updatedate<='"
		 * ).append(find_DateStart).append(" 00:00:00'")
		 * .append(" and updatedate>='"
		 * ).append(find_DateEnd).append(" 23:59:59'") .append(";");
		 * 
		 ...
		 * //System.out.println(
		 * CoverDate(ReturnSysTime("yyyy-MM-dd HH:mm:ss"),"yyyy-MM-dd")); try {
    ...
    public static String getROOTPath() {
        UtilFuns uf = new UtilFuns();
        return uf.getClass().getResource("/").getPath()
                .replace("/WEB-INF/classes/", "/").substring(1);
    }
